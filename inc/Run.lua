...
user.Source  = DirFol
local url , res = https.request('https://api.THTSS.com/GetID/?Array='..JSON.encode(user))
if res ~= 200 then
print(res)
...
boos = Token:match("(%d+)")
redis:mset(
boos..":VERSION",GetUser.information.Source_version,
boos..":SUDO_ID:",GetUser.information.id,
boos..":DataCenter:",GetUser.information.DataCenter,
boos..":UserNameBot:",BOT_User,
boos..":ApiSource",GetUser.information.WebSite,
boos..":NameBot:","النيزك",
"TH3boos_INSTALL","Yes"
)
redis:hset(boos..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {} 
...
boos = Token:match("(%d+)")
our_id = tonumber(boos)
Apiboos = redis:get(boos..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(boos..":UserNameBot:")
SUDO_ID = tonumber(redis:get(boos..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_ID =  tonumber(redis:get(boos..":SUDO_ID:"))
SUDO_USER = redis:hgetall(boos..'username:'..SUDO_ID).username
version = redis:get(boos..":VERSION")
DataCenter = redis:get(boos..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..boos..')\027[0;32m\n'
...
..'¦ Api_Src->: \27[1;34m'..Apiboos..'\027[0;32m\n'
...
if not redis:get(boos..":Check_Bot:"..W) then
Rgz,res=https.request(Apiboos..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(boos..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(boos..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(boos..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(boos..'group:add'..msg.chat_id_) then 
...
if msg.GroupActive and redis:get(boos..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end
...
msg.Rank = 1
elseif redis:sismember(boos..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(boos..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ اساسي'
msg.TheRank = 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(boos..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(boos..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(boos..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(boos..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(boos..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(boos..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(boos..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(boos..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(boos..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(boos..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(boos.."lock_Add"..msg.chat_id_) then
...
if msg.Special or msg.Admin then redis:srem(boos..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(boos..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(boos..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.boos then 
if msg.text and ScriptFile.iboos then
	Xboos = ScriptFile.boos
	local list = redis:hgetall(boos..":AwamerBotArray:"..msg.chat_id_)
	for boos2,k in pairs(list) do
		Text = msg.text
		Text2 = k
		if Text:match(boos2) then 
		local AmrOld = redis:hgetall(boos..":AwamerBotArray2:"..msg.chat_id_)
		amrnew = "" amrold = ""
		...
		GetMsg = ScriptFile.iboos(msg,{Text:match(Text2)})
		if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(boos) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iboos(msg,{Text:match(boos)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",boos.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dboos then
if ScriptFile.dboos(msg) == false then
return false
end
...
if Plug.boos then 
if msg.text and Plug.iboos then
for k, boos in pairs(Plug.boos) do
if msg.text:match(boos) then
local GetMsg = Plug.iboos(msg,{msg.text:match(boos)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",boos.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source boos")
end 

end
else
print("The File Script.lua Not Runing in The Source boos")
end
end
...
function tdcli_update_callback(data)
	...
	redis:srem(boos..'users',msg.sender_user_id_)
	end
...
if redis:get(boos..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(boos..":propin"..msg.chat_id_)
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) vardump(d)end,nil)   
	...
	if redis:get(boos..'group:add'..data.chat_.id_) then
	redis:set(boos..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	elseif data.ID == "UpdateChannel" then  
	if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
	if redis:get(boos..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(boos..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(boos..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	boos..'group:name'..k,IDS.Title,
	boos..'num_msg_max'..k,5,
	boos..'group:add'..k,true,
	boos..'lock_link'..k,true,
	boos..'lock_spam'..k,true,
	boos..'lock_webpage'..k,true,
	boos..'lock_markdown'..k,true,
	boos..'lock_flood'..k,true,
	boos..'lock_bots'..k,true,
	boos..'mute_forward'..k,true,
	boos..'mute_contact'..k,true,
	boos..'mute_document'..k,true,
	boos..'mute_inline'..k,true,
	boos..'lock_username'..k,true,
	boos..'replay'..k,true
	)
	redis:sadd(boos..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(boos..'username:'..ID,'username',user)
	redis:sadd(boos..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(boos..'username:'..ID,'username',user)
	redis:sadd(boos..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(boos..'username:'..ID,'username',user)
	redis:sadd(boos..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(boos..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(boos..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(boos..'username:'..data.user_.id_).username
	...
	redis:hset(boos..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
